...
@DETECTOR.register_module(module_name='gig_base_nodecls')
class GIGBaseNCDetector(AbstractDetector):
    ...
    def build_backbone(self, config):
        ...
        if config['use_fake_penalty']:
            fake_penalty_config = config['fake_penalty_config']
            print(f"[INFO] Using fake penalty ... {fake_penalty_config}")
        else:
            fake_penalty_config =  None
        backbone = GIG_Model(num_class=2, num_segment=config['clip_size'], add_softmax=False,
                             pixel_in_channels=model_config['pixel_in_channels'],
                             pixel_hidden_channels=model_config['pixel_hidden_channels'],
                             pixel_out_channels=model_config['pixel_out_channels'],
                             edge_refine=model_config['edge_refine'], feature_dim=model_config['feature_dim'],
                             fake_penalty=fake_penalty_config)
...
class GIG_Model(nn.Module):
    def __init__(self,
        ...
        self.pixel_in_channels = pixel_in_channels
        self.pixel_hidden_channels = pixel_hidden_channels
        self.pixel_out_channels = pixel_out_channels
    ...
    def build_model(self):
        ...
        Construct the model.
        """
        self.base_model = scnet50_v1d(self.num_segment, pretrained=True, pixel_in_channels=self.pixel_in_channels, pixel_hidden_channels=self.pixel_hidden_channels,
                                      pixel_out_channels=self.pixel_out_channels, edge_refine=self.edge_refine, feature_dim=self.feature_dim)
    ...
    def forward(self, x):
        ...
        if self.fake_penalty is not None:
            # ori_mean = out.mean(1, keepdim=False)
            ori_pred = torch.softmax(out, dim=2)
            out = torch.empty_like(ori_pred)
            w = torch.tensor(self.fake_penalty['w'])
            lamda = torch.tensor(self.fake_penalty['lamda'])
            threshold = torch.tensor(self.fake_penalty['threshold'])
            n, t=out.size()[0:2]
            for batch in range(n):
                for dim in range(self.num_class):
                # for dim in [1]:
                    for time in range(t):
                        if time == 0:
                            w_t = 1
                        else:
                            diff = torch.abs(ori_pred[batch, time, dim] - ori_pred[batch, time - 1, dim])
                            if diff > threshold:
                                w_t = 1 + w * diff
                            else:
                                w_t = 1
                        out[batch, time, dim] = w_t * torch.exp(lamda * ori_pred[batch, time, dim])
                        # print(dim)
                        # print(ori_pred[batch, time])
                        # print(out[batch, time])
                        # print(torch.softmax(out[batch, time], dim = 0))
                        # print('---')

            # pen_out = out.mean(1, keepdim=False)  # [n, num_class]
            # # out = torch.softmax(out, dim=1)
            # out = ori_mean + pen_out
...
class SCNet(nn.Module):
    def __init__(self, num_segments, block, layers, pixel_in_channels, pixel_hidden_channels, pixel_out_channels,
                 edge_refine, feature_dim, groups=1, bottleneck_width=32,
                 num_classes=2, dilated=False, dilation=1,num_classes_sc=1000,
        ...
        self.fc = nn.Linear(512 * block.expansion, num_classes_sc)
        ...
        self.gigfake = GIGFake(pixel_in_channels = pixel_in_channels,
                               pixel_hidden_channels = pixel_hidden_channels,
